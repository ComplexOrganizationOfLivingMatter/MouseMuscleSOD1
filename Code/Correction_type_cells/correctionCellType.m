function correctionCellType(path_folder)
%This function is necessary in cases where cell-type property wrongly
%labelled. This function assumes a curated segmentation of muscle fibres segmentation 
    
    load(fullfile(path_folder,'Data_image','Data_cc.mat'))
    load(fullfile(path_folder,'Data_image','Needed_images.mat'))

    if exist(fullfile(path_folder,'slow_fast_cells_correction.mat'),'file')
        load(fullfile(path_folder,'slow_fast_cells_correction.mat'),'slow_cells','fast_cells')
    end
    
    %this function let modify type of cells
    [folder,name,~] = fileparts(path_folder);
    ...
    %% SAVING
    %%Save 67 cc (There are more feature extracted, you could save what you want)
    stringres=fullfile(path_folder,'Data_image','Results_67_cc.mat');
    ...
    stringres=fullfile(path_folder,'Data_image','Data_cc.mat');
    save (stringres,'slow_cells','fast_cells','old_slow_cells','old_fast_cells','cells_L','valid_cells','no_valid_cells','valid_cells_neigh_of_neigh','no_valid_cells_neigh_of_neigh','valid_cells_4','valid_cells_5','neighs_real','sides_cells')
