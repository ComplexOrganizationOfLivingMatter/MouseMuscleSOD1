function [valid_cells]=Extraction_69ccs(filePath)
...
%      cd ..\Ccs_extraction
    ...
    path_data = strrep(filePath,'Photos','Processed_images');
    load(fullfile(path_data, 'Data_image', 'Needed_images.mat'))

    %load real image
    splittedPath = strsplit(path_img,filesep);
    name=splittedPath{end};
    Img=imread(fullfile(path_data,[name '.jpg']);
    ...
    stringres=fullfile(path_data, 'Data_image','Results_69_cc.mat');
    ...
    stringres=fullfile(path_data, 'Data_image','Data_cc.mat');
