    ...
    for i=1:expansion
        ...
        if isempty(BetterPCAs)==0
            ...
            if isempty(newPCARow)==0
                BetterPCAs(count+i,1)=Ratio_UMAP(1,maxUMAPIndex);
                for j=2:size(BetterPCAs_bef,2)
                    BetterPCAs(count+i,j)=BetterPCAs_bef(rowCCs,j);
                end
                BetterPCAs(count+i,size(BetterPCAs_bef,2)+1)=Ratio_UMAP(end,maxUMAPIndex);
        ...
        %When BetterPCAs isempty add row
            ...
            [BetterPCAs(count+i,1) maxUMAPIndex]=max(auxiliar);
