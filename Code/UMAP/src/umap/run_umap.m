...
%       supervised by EPP and confirm dissimilarity between supervised subsets
%       and previously classified subsets in the test set sample.
%
%       run_umap('sampleBalbcLabeled12k.csv', 'template_file', 'ustBalbc2D.mat', 'label_column', 'end', 'match_scenarios', 4,  'see_training', true);
%
%   18. Same as example 17 but add a false positive/negative plot
%
%       run_umap('sampleBalbcLabeled12k.csv', 'template_file', 'ustBalbc2D.mat', 'label_column', 'end', 'match_scenarios', 4, 'match_histogram_fig', false, 'see_training', true, 'false_positive_negative_plot', true);
%
...
%
%       run_umap('s2_samusikImported_29D.csv', 'template_file', 'ust_s1_samusikImported_29D_30nn_2D.mat', 'label_column', 'end', 'label_file', 's2_samusikImported_29D.properties', 'match_scenarios', 4,  'match_histogram_fig', false, 'see_training', true, 'false_positive_negative_plot', true, 'match_supervisors', [1 3 4]);
%
...
args=UmapUtil.Initialize(varargin{:});
args=UmapUtil.CheckArgs(args);
clusterIdentifiers=[];
...
if beGraphic
    ...
    if isempty(csv_file_or_data)
        [answer, cancelled]=Gui.Ask(Html.Wrap(...
            ['Options for getting example files from the Herzenberg '...
            'Lab @ Stanford University.']),...
            {'Download from Google Drive and exit', ...
            'Download from CytoGenie.org, unzip & keep running!!'}, ...
...
if ~isempty(template_file)
    ...
    if isempty(umap)
        ...
        if ~isempty(umap.supervisors)
            ...
            sprv.overrideColors(args.color_file);
            if beQuiet
...
elseif strcmpi('Mex', args.method)
    ...
    if ismac
        exe=fullfile(curPath,'mexStochasticGradientDescent.mexmaci64'); 
    else
        exe=fullfile(curPath,'mexStochasticGradientDescent.mexw64');
    end
    if ~exist(exe, 'file')
        UmapUtil.OfferFullDistribution(true)
        if ~exist(exe, 'file')
            if ~askYesOrNo('Run slower with Java?')
                if beGraphic
                    close(fig);
                end
                return;
...
if args.label_column>0 
    if isSupervising && ~isempty(args.label_file) && isempty(testSetLabels)
        warning(['label_file has no affect when reducing '...
        ...
        if ~isempty(testSetLabels)
            ...
            if hasLabels
                ColorsByName.Override(labelMap, args.color_file);
            end
        end
        if halt
            delete(fig);
    ...
    function clues=doClusters(data)
        if isempty(data)
            ...
            [~, clues]=Density.FindClusters(reduction, ...
                args.cluster_detail{1}, args.cluster_method_2D, true,...
